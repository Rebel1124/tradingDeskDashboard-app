...
def candles(currencyPairs, timeLag):
    ...
    for currencyPair in currencyPairs:
        ...
        candlesPair['time'] = candlesPair.apply(lambda x: (datetime.datetime.time(x['time']) + timedelta(hours=2, minutes=0)), axis=1)
...
def currencyUSD(tickerStrings):
    ...
    df.index = df.apply(lambda x: (datetime.datetime.time(x['t']) + timedelta(hours=2, minutes=0)), axis=1)
    df = df[['c']]
